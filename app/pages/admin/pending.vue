<template>
  <v-dialog v-model="loading_modal" max-width="320">
    <v-list class="py-2" color="green" elevation="12" rounded="lg">
      <v-list-item title="已提出審核通過請求...">
  ...
  <v-dialog v-model="approve_modal" width="auto">
    <v-card max-width="400" prepend-icon="mdi-help" text="是否確定通過" title="請確認是否通過此文章">
      <template v-slot:actions>
        <v-btn class="ms-auto" text="通過" @click="approve_post(1)"></v-btn>
      </template>
    </v-card>
  </v-dialog>
  <v-dialog v-model="error_modal">
    <v-card class="mx-auto" prepend-icon="mdi-alert-circle" width="400" color="red">
          ...
          <v-list lines="two">
            ...
            <v-data-table :items="review_posts" :headers="headers" item-key="title">
              <template v-slot:item.actions="{ item }">
                <v-btn icon="mdi-check" variant="text" @click="approve_modal = true" />
...
const { data } = useFetch("/api/admin/articles/list")
